import { takeEvery, select, put, fork, take } from 'redux-saga/effects'
...
function* gameOverWatcher({ payload: { x, y } }) {
  const exitPosition = yield select(getExitPosition)

  if (exitPosition.x === x && exitPosition.y === y) yield put(gameOver())
}
...
export function* tutorial1() {
  ...
  while (userMoves.right < 3 || userMoves.down < 1) {
    ...
    if (action.type === moveRight.toString()) userMoves.right += 1
    else if (action.type === moveDown.toString()) userMoves.down += 1
  }
...
export default function*() {
  yield takeEvery([moveLeft, moveRight, moveDown, moveUp], moveUserWatcher)

  yield takeEvery(changePosition, gameOverWatcher)
